...
main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-d") == 0) {
            ...
            userProgram.push_back(userProgName);
            i++;
        }
    ...
    if (userProgName != NULL) {
        if (userProgram.size() > 0 ) {
            ...
            for (int i = 0; i < userProgram.size() ; i++) {
                cout << i << " " << userProgram.size() << endl;
                string str(userProgram[i]);
                string sub = str.substr(8);

                char *tName = new char[str.length() + 1];
                strcpy(tName, sub.c_str());

                Thread *t = new Thread(tName);
                threads->Append(t);
                kernel -> threadMap.insert({sub, t});
                kernel -> currentThread -> Yield();
            }

            ListIterator<Thread*> *it = new ListIterator<Thread*>(threads);
            for (int i = 0; i < userProgram.size() ; i++) {
                Thread *t = threads->RemoveFront();
                t->Fork((VoidFunctionPtr) RunUserProg, (void *) userProgram[i]);
            }
        }
