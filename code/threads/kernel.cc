...
Kernel::Kernel(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
	    ...
	    i++;
         ...
         //ASSERT(i + 1 < argc);
        cout<<"entering quantum"<<endl;
         if(!(argv[i + 1] == NULL || atoi(argv[i + 1]) <= 10)){
             quantumNo = atoi(argv[i + 1]);
             cout<<"Assigned quantum with quantumNo: "<<quantumNo<<endl;
             flag = FALSE;
         }
         i+=2;
...
Kernel::Initialize()
{
    ...
    if(flag) {
        cout<<"Quantum Too Small"<<endl;
        cout<<"Assigned quantum with quantumNo: "<<quantumNo<<endl;
    }
    alarm = new Alarm(randomSlice, quantumNo);	// start up time slicing
